...
if(isset($_GET['no'])) {
    ...
    $sql = "SELECT Product_name FROM `product` WHERE Product_code LIKE '$no'";
...
if(isset($_POST['pno'])) {
    ...
    if($_POST['quan']=='')
        ...
        $quan = $_POST['quan'];
    $sql = "UPDATE `product` SET `Product_Quantity` = Product_Quantity+$quan WHERE `product`.`Product_code` = $pno";
    $stmt = $mysql_db->query($sql);



    $date = date('Y-m-d');
    $sql ="SELECT PNo FROM Product WHERE Product_code LIKE $pno";
...
//    exit();
    $stmt = $mysql_db->query($sql);
//    $sql = "UPDATE `product` SET `Product_Quantity` = Product_Quantity + $quan WHERE `product`.`PNo` = $pno";
//    $stmt = $mysql_db->query($sql);
//    echo $sql;
//    exit();
...
<div class="row">
    <div class="col-lg-12 " style="vertical-align:middle;text-align:center">
        <h1 class="page-header" align="center">Add <?= $row['Product_name']?> quantity</h1>
